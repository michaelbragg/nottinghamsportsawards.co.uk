 ...
 *
 * @param string $taxonomy_slug Award slug.
 */
...
if ( ! function_exists( 'the_custom_archive_description' ) ) :

	/**
	 * Display custom option content with optional formatting
	 *
	 * @param  [string] $before HTML string to display before the content.
	 * @param  [string] $after  HTML string to display after the content.
	 * @return [string|false]         HTML string to display content.
	 */
	function the_custom_archive_description( $before = null, $after = null ) {

		$archive_type	= get_query_var( 'post_type', false );
		if ( false === $archive_type ) {
			return false;
		}

		$archive_options	= get_option( $archive_type . '-options', false );

		if ( false === $archive_options ) {
			return false;
		}

		$option_name = '_' . str_replace( '-', '_', $archive_type ) . '_options-description' ;

		if ( empty( $archive_options[ $option_name ] ) ) {
			return false;
		}

		echo wp_kses_post( $before . wpautop( $archive_options[ $option_name ] ) .  $after );
