...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['action'] ) && $_POST['action'] == 'update-user' ) {
	...
	if ( ! empty( $_POST['pass1'] ) && ! empty( $_POST['pass2'] ) ) {
		...
		} else { 			$error[] = __( 'The passwords you entered do not match.  Your password was not updated.', 'tm-events-2016' ); }
	}

	/* Update user information. */
	if ( ! empty( $_POST['email'] ) ) {
		if ( ! is_email( esc_attr( $_POST['email'] ) ) ) {
			$error[] = __( 'The Email you entered is not valid.  please try again.', 'tm-events-2016' ); } elseif ( email_exists( esc_attr( $_POST['email'] ) ) != $current_user->id ) {
			$error[] = __( 'This email is already used by another user.  try a different one.', 'tm-events-2016' ); } else {
